 ...
 * Fecha: 11/04/2026
 * Descripción: Servicio de encriptación y desencriptación de datos.
 * Última modificación:
 * Modificado:
 */
...
} from './interfaces/encriptador.service.interface';
import { environments } from '../../../environments/environment';

/**
 * Servicio de encriptación y desencriptación de datos.
 *
 * Este servicio centraliza la lógica para cifrar y descifrar información
 * sensible utilizando el algoritmo AES en modo CBC con padding PKCS7.
 *
 * Se utiliza en la aplicación para proteger datos en tránsito o en reposo,
 * asegurando que solo clientes autorizados con la llave y vector de inicialización
 * correctos puedan acceder a la información.
 */
@Injectable()
export class EncryptionService {
  ...
  public async cifrarDatos(payload: { key?: string; iv?: string; data: any }): Promise<encryptResponseInterface> {
    try {
      ...
      const data = {
        key: payload?.key || environments.deployment.keys.key,
        iv: payload?.iv || environments.deployment.keys.iv,
        data: _data
      };
      ...
      return { error: true, message: 'Data encrypt failed', cipher: '' };
    }
  }
  ...
  private async encrypt(cipher: EncryptServiceInterface): Promise<encryptResponseInterface> {
    try {
      ...
      return { error: false, message: 'Data encrypt success', cipher: encryption.toString() };
    } catch (e: any) {
      return { error: true, message: 'Data encrypt failed', cipher: '' };
    }
  }
  ...
  private async decrypt(data: DecryptServiceInterface): Promise<encryptResponseInterface> {
    try {
      ...
      return { error: false, message: 'Data decrypt success', data: rps };
    } catch (e: any) {
      return { error: true, message: e.message, data: null };
    }
  }
